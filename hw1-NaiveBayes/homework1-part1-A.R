...
{wtd<-createDataPartition(y=bigy, p=.8, list=FALSE) #creat test/training partition, a vector of index?
  ...
  yplog <- rowSums(log(yp))+(log(nrow(ptregs)/nrow(ntrbx)))
  ...
  ynlog <- rowSums(log(yn))+(log(nrow(ntregs)/nrow(ntrbx)))
...
#--------------  
  ...
  test.yplog <- rowSums(log(test.yp))+(log(nrow(ptregs)/nrow(ntrbx)))
  ...
  test.ynlog <- rowSums(log(test.yn))+(log(nrow(ntregs)/nrow(ntrbx)))
